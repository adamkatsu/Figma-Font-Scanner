...
| **1.2.5** | Jul 10, 2026 | `b169da2` | Released |
| **1.2.6** | Jul 11, 2026 | — | Released |
...
- **Security** – Removed Google Fonts Web Fonts API key from the UI; Google family classification uses a static list generated by `scripts/fetch-google-fonts.mjs` (`npm run update:google-fonts`). Dropped `www.googleapis.com` from `manifest.json` network access.
- **UI modularization** – Source UI lives under `ui/index.html` + `ui/src/*`; `scripts/build-ui.mjs` (esbuild) inlines CSS/JS into root `ui.html`. Added `esbuild` and `build:ui` / `build:code` scripts.
- **Scan scalability** – Session cache for `listAvailableFontsAsync`; `scan-progress` with UI yield so counts like `12/341` paint; lighter `scan-result` (no full `systemFonts` every scan); `get-system-fonts` for Replace autocomplete; large-page toast shortened.
- **UX polish** – Select-fonts control uses `fa-arrow-pointer`; scan overlay shows immediately; TypeScript/`tsconfig` fixes for Figma typings and `moduleResolution: bundler`.
- **Docs** – Updated `AGENTS.md`, `README.md`, and release rules for the new UI build path (`ui/index.html` + regenerate `ui.html`).

**Files changed:** `code.ts`, `ui/`, `scripts/`, `ui.html`, `manifest.json`, `package.json`, `package-lock.json`, `tsconfig.json`, `README.md`, `VERSION.md`, `AGENTS.md`, `.cursor/rules/git-push-release.mdc`

---

## v1.2.5 — Released

**Commit:** `b169da2` — *docs: use descriptive commit messages for releases* (Jul 10, 2026)
...
2026-07-10  b169da2  release v1.2.5 — descriptive release commit messages         [kemonn98]
2026-07-11  release v1.2.6 — security, modular UI, scan progress                 [kemonn98]
```
